      ...
      <div className="w-full flex justify-center relative z-10">
        <div className="w-full max-w-sm px-4 py-6 flex flex-col items-center">
          <motion.div
            ...
            className="text-center px-5 py-6 rounded-3xl bg-white/70 backdrop-blur-md shadow-lg"
          >
            <h1 className="text-3xl sm:text-4xl font-extrabold text-pink-700 mb-4">
              🎉 Happy Birthday, Anushka! 🎂
            </h1>
            <p className="text-lg sm:text-xl text-pink-700 font-semibold">
    ...
    return (
      <div className="w-full flex flex-col items-center relative">
        ...
        <div className="absolute top-0 left-1/2 transform -translate-x-1/2 flex flex-col items-center z-0 pointer-events-none">
          ...
          {!isLast && <div className="flex-1 w-0.5 bg-pink-200 mt-1 mb-1" />}
        </div>
        ...
        {openNode === node.id ? (
          ...
          >
            <img
              ...
              className="w-full max-h-[60vh] sm:max-h-[70vh] object-contain rounded-xl shadow-2xl bg-white"
            />
          ...
          <motion.div
            ...
            className="w-20 h-20 sm:w-24 sm:h-24 bg-pink-500 rounded-xl shadow-lg flex items-center justify-center text-white text-2xl sm:text-3xl cursor-pointer z-10"
          >
            🎁
          </motion.div>
        )}
    ...
    return (
      <div className="w-full flex flex-col items-center relative z-10">
        ...
        <motion.div
          ...
          className="w-52 h-52 sm:w-56 sm:h-56 bg-gradient-to-br from-pink-600 to-purple-600 rounded-2xl shadow-2xl flex items-center justify-center text-white text-4xl sm:text-5xl cursor-pointer z-10 mt-2"
        >
          🎁
        </motion.div>
      </div>
...
export default function BirthdayRoadmapComponent() {
  const photos = useMemo(() => Array.from({ length: 16 }, (_, i) => `/photos/photo${i + 1}.jpg`), []);

  const nodes = useMemo(() => [
    { id: "hero", type: "hero" },
    ...Array.from({ length: 12 }, (_, i) => ({ id: `p-${i}`, type: "photo", index: i })),
    { id: "big", type: "big" },
  ], []);
    ...
    const onScroll = () => {
      if (!ticking) {
        window.requestAnimationFrame(() => {
          ...
          setScrollProgress(h <= 0 ? 0 : Math.min(1, window.scrollY / h));
          ticking = false;
        });
        ticking = true;
      }
  ...
  const floatingItems = useMemo(() =>
    Array.from({ length: 55 }).map(() => ({
      ...
      emoji: Math.random() > 0.6 ? "❤️" : Math.random() > 0.5 ? "🌹" : Math.random() > 0.5 ? "🎈" : "✨"
    }))
  , []);
  ...
  return (
    <div
      style={{ backgroundImage: `linear-gradient(to bottom, ${bgTop}, ${bgBottom})` }}
    ...
    >
      ...
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        {floatingItems.map((item, i) => (
          <motion.div
            ...
            transition={{ duration: item.dur, delay: item.delay, repeat: Infinity, ease: "linear" }}
            className="absolute"
          >
            {item.emoji}
          </motion.div>
      ...
      <div className="fixed bottom-4 right-2 sm:bottom-6 sm:right-6 z-50 flex items-center space-x-2 sm:space-x-3 bg-white/80 backdrop-blur rounded-full px-2 py-1 sm:px-3 sm:py-2 shadow">
        <button onClick={toggleAudio} className="px-2 py-1 sm:px-3 sm:py-2 rounded-full bg-pink-500 text-white text-xs sm:text-base">
          ...
          >
            ...
            >
              <div className="flex justify-between items-center mb-4">
                ...
                <button onClick={() => setGalleryOpen(false)} className="px-4 py-2 rounded bg-pink-100 text-pink-700">Close</button>
              </div>
              <div className="grid grid-cols-2 gap-4 sm:gap-6">
                {photos.map((p, i) => (
                  <img key={i} src={p} alt={`g-${i}`} className="w-full h-40 sm:h-48 object-cover rounded-xl shadow-lg" />
                ))}
              </div>
            </motion.div>
          </motion.div>
